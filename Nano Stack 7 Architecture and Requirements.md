# Nano Stack 7 — Architecture and Requirements
...
**Definition of done for the PoC:** runs and is demoable end-to-end on the dev PC and a Windows 11 VM (VirtualBox), both talking to the server stack running via Docker Desktop or the LAN Portainer host. No installer/packaging polish required at this stage (see Section 13).
...
### 13.3 Client Test Targets

- Local dev PC (Windows).
- A separate Windows 11 VM running under **VirtualBox**.

### 13.4 Dev PC Software Requirements (Windows)

| Category | Tool | Purpose |
|---|---|---|
| Core toolchain | Rust via `rustup` (stable, `x86_64-pc-windows-msvc` target, `rustfmt` + `clippy`) | Primary language for server and client. |
| Core toolchain | Visual Studio Build Tools (C++ workload) | MSVC linker required by Rust on Windows; native deps (`snow`, Windows Service bindings). |
| Core toolchain | `protoc` (Protocol Buffers compiler) | Required by `prost` to compile `.proto` files. |
| Core toolchain | Git + working GitHub SSH key | Source control for `nano-stack-7`, including working across the `main`/`dev` branches. |
| Server dev/test | Docker Desktop (WSL2 backend) | Build/run `deploy/docker-compose.yml` locally before merging to `main` for Portainer to pick up. |
| Server dev/test | PostgreSQL client tools (`psql` or DBeaver/TablePlus) + `sqlx-cli` | Schema inspection and running migrations. |
| Client dev/test | Windows 11 SDK | Windows Service APIs, Winget invocation, WMI-based inventory, notification/toast APIs. |
| Client dev/test | WiX Toolset or `cargo-wix` | Client installer packaging — not required for PoC done-criteria, but useful to have ready ahead of Phase 2/3. |
| Client dev/test | Sysinternals suite (Process Explorer, Process Monitor) | Debugging service install/start issues; watching daemon memory/handle usage against the ~10–20MB RSS target. |
| Client dev/test | VirtualBox | Hosts the Windows 11 VM used as a second client test target. |
| Cross-cutting | VS Code + `rust-analyzer` + CodeLLDB | Primary IDE/debugger. |
| Cross-cutting | `cargo-watch` | Auto-rebuild/rerun during daemon iteration. |
