...
### DEV environment moved to two dedicated VMs: `vm-lab1` (client) + `vm-docker` (server)

User is decommissioning `lab1` and, separately, retiring the WSL2 + rootless Podman server stack on the workstation, replacing both with dedicated VMware VMs on the workstation's VMnet8 NAT network: `vm-lab1` (Windows 11 Pro, client dev box, continuing the migration started the previous day) and `vm-docker` (new — Ubuntu 26.04, Docker CE + Portainer, the new server dev box). Confirmed explicitly: these two VMs are now the default NS7 dev environment; PROD deployment/validation is human-only from here on; orchestration combines SSH (scripted work) and VNC (interactive work).

- **Wrote a minimal RFB/VNC client from scratch** (`rfb.py`, scratch tooling, not checked into the repo) to drive `vm-lab1`'s console via VMware's built-in per-VM VNC server (enabled by the user, no password, port 5900) — raw encoding, no auth, screenshot capture (hand-rolled PNG encoder via `zlib`, no Pillow dependency) plus keyboard/pointer input (named keys, Shift-mapped text typing, click). Run from WSL2, since WSL2's mirrored networking reaches the VM's VNC port over loopback but not arbitrary VMnet8 peers directly. Used it to watch and verify the user's own elevated-console run of the `bootstrap-devbox.ps1` commands on `vm-lab1` (OpenSSH Server install, `administrators_authorized_keys` write, firewall rule) — confirmed via screenshots at each stage and a final verification pass (`Get-Content administrators_authorized_keys`, `IsInRole('Administrators')`, `Get-Service sshd`) all correct.
- **Generated a dedicated keypair** `~\.ssh\id_ed25519_vmlab1` for `vm-lab1`; updated `scripts/devbox.config.ps1` and `scripts/dev-stack.ps1`'s status output to the new static IP `192.168.155.10` (previously the DHCP-leased `192.168.155.128`).
- **vm-docker stood up**: user supplied `sysadmin`/(password redacted, shared in chat) for the new VM at `192.168.155.129`. Probed and confirmed identity/state before acting rather than assuming: `hostnamectl` → Ubuntu 26.04 LTS "Resolute Raccoon", DHCP address on `ens33`, gateway `192.168.155.2`, no Docker installed, `sudo` requires a password.
  - **Installed PuTTY** (`winget install PuTTY.PuTTY`) for `plink`/`pscp`, after Windows' native `ssh.exe` (9.5p2) and Git for Windows' bundled `ssh.exe` (10.3p1) both failed to authenticate with a freshly generated key (`~\.ssh\id_ed25519_vmdocker`) against this box, and against `vm-lab1` too — see the dedicated finding below.
  - **Set a static IP**: wrote a netplan config (`ens33`, `192.168.155.20/24`, gateway `192.168.155.2`, DNS `192.168.155.2`, MAC-pinned), backed up the original `/etc/netplan/00-installer-config.yaml` to `/tmp`, validated with `netplan generate` before applying, then `netplan apply` — confirmed the box came back on the new address and the old one dropped.
  - **Installed Docker Engine** via Docker's official apt repo rather than the convenience script, for reproducibility: added `/etc/apt/keyrings/docker.asc`, and — since Ubuntu 26.04 is newer than any codename Docker's apt repo currently serves — added a fallback that probes for a matching suite and falls back to `noble` (24.04) if none exists yet (the repo serves packages, not a distro-specific build, so this is safe). Installed `docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin`, enabled+started the service, added `sysadmin` to the `docker` group, verified with `docker version` (Server 29.7.1), `docker compose version` (v5.4.0), and a `docker run --rm hello-world`.
  - **Installed Portainer CE**: `docker volume create portainer_data` + `docker run -d --restart=always` bind-mounting `/var/run/docker.sock`, ports 8000/9000/9443. Verified reachable from the workstation on `9443`.
  - Cleaned up: removed the temporary sshd debug-logging drop-in and the staged netplan file used during diagnosis.
- **Found and diagnosed a cross-box SSH key-auth failure** (`vm-docker` and `vm-lab1` both): every OpenSSH client available in this environment — Windows-native OpenSSH 9.5p2/LibreSSL, Git for Windows' bundled OpenSSH 10.3p1/OpenSSL — fails "Permission denied (publickey,password[,keyboard-interactive])" against both boxes' SSH servers. Root-caused with `ssh -vvv`: the client offers the key, the server responds "Server accepts key", the client then attempts `sign_and_send_pubkey: using publickey-hostbound-v00@openssh.com`, and silently fails to produce a signature (`we did not send a packet, disable method`) — reproduced identically via `ssh-agent` (rules out a raw-key-file-specific cause). Confirmed neither `sshd_config` (checked full effective config via `sshd -T` on both boxes, including a temporary `LogLevel DEBUG3` pass on `vm-docker` cross-checked against `journalctl`) exposes a toggle for the extension — it's negotiated at the KEX level. Left unresolved pending a client build without the bug; documented in `scripts/devserver.config.ps1` and README Section 13.6 so it isn't re-discovered from scratch.
  - Workaround adopted for `vm-docker`: password auth via `plink -batch -hostkey <pinned fingerprint> -pw`, which doesn't hit the buggy code path. New **`scripts/devserver.config.ps1`** centralizes this (host/user/remote dir/pinned host-key fingerprint, `Invoke-DevServer`/`Copy-ToDevServer` helpers, password taken via `Read-Host -AsSecureString` per invocation — deliberately never stored in the repo or on disk).
  - Workaround in progress for `vm-lab1` (Windows-to-Windows, so WinRM is a native alternative the user proposed): enabled `Enable-PSRemoting -Force -SkipNetworkProfileCheck` on `vm-lab1` via the VNC console (confirmed: WinRM firewall rule active, `Test-WSMan localhost` responds). Client-side setup on the workstation (`Start-Service WinRM`, `Set-Item WSMan:\localhost\Client\TrustedHosts`) requires elevation my shell doesn't have — handed the exact commands to the user to run themselves, consistent with the existing pattern for `setup-dev-networking.ps1`. Not yet completed as of this entry.
  - Explicitly declined one path: setting a new `sysadmin` password on `vm-lab1` myself via the VNC console (to unblock `plink`, matching the `vm-docker` pattern) was blocked by the harness's own permission classifier as an account-credential change, and I did not attempt to route around it — surfaced the tradeoff to the user instead, who chose WinRM.
- **Rewrote `scripts/dev-stack.ps1`** to target `vm-docker` instead of WSL2/podman-compose on the workstation: syncs `Cargo.toml`/`shared`/`ns7-server` via `pscp`, runs `docker compose -p ns7dev -f docker-compose.yml -f docker-compose.dev.yml <action>` over `plink`. Dropped the `api` action (Podman Desktop integration — moot now that Portainer runs directly on the Docker host). Compose files themselves needed no changes; `docker compose` reads the same spec `podman-compose` did.
- **Updated README** for the new topology:
  - Section 13.3: `vm-lab1` described as a VMware VM on VMnet8 rather than a physical LAN box reached over Tailscale.
  - Section 13.4: split rewritten as `vm-lab1` (Windows toolchain) / `vm-docker` (Docker + Portainer) rather than dedicated-box / WSL2; removed WiX and Windows SDK from the dev-box requirement list (installer packaging is CI's job, not the dev box's).
  - Section 13.5: dev/prod table updated (server stack, Admin Console, visual UI, client target all point at the new boxes); added a "Prod is human-operated only" line and marked the release-flow's deploy/validate steps `(human)`; scripts table updated for `devserver.config.ps1` and the new `dev-stack.ps1`; **removed the Podman Desktop subsection** entirely (Portainer replaces it).
  - Section 13.6: added a "VNC as a second channel" subsection documenting the RFB client, the SSH/VNC split of responsibilities, and the `publickey-hostbound` finding.
  - Left historical statements in Section 11.1 referencing `lab1` untouched — those are records of where something was verified at the time, not current-state documentation.
- Deliberately kept `scripts/setup-dev-networking.ps1` rather than deleting it, but demoted it in the docs to "only matters if the Admin Console needs to run on the workstation directly again" — the server stack moving to `vm-docker` (its own IP, its own published ports) makes the WSL2-mirrored-networking problem it solved moot for the normal flow, but it's still valid if that flow is ever needed.
- **Validated the rewritten `dev-stack.ps1` end-to-end** against the live `vm-docker` box (not just parse-checked): `status` (connects, reports gracefully before anything is deployed), then `up` — synced source including `ns7-client` (initially dropped from the sync list by mistake; the workspace `Cargo.toml` needs every member crate present for `cargo build -p server` to resolve even though the server image doesn't use `ns7-client`'s code — caught immediately by the Dockerfile's `COPY ns7-client` step failing, fixed, re-ran clean), built the server image from source inside the container (multi-stage: Rust backend + Node frontend), started `postgres`+`server`, confirmed both containers `Up` with ports published on `0.0.0.0`, and confirmed the Admin Console answers `HTTP 200` on `http://192.168.155.20:8080`. Added an `NS7_DEVSERVER_PASSWORD` environment-variable fallback to `Read-DevServerPassword` for exactly this kind of non-interactive verification (a real console still gets the `Read-Host` prompt by default).

### Resolved `vm-lab1` automation: key-auth bug is real, but password auth was never affected

User asked to revisit SSH for `vm-lab1` rather than settle for VNC-only, and separately confirmed they can `ssh sysadmin@192.168.155.10` "smoothly" from their own session — which turned out to be the key fact: that almost certainly uses password auth (no `-i` key, interactive prompt), which has no signing step and so was never exposed to the `publickey-hostbound@openssh.com` bug documented earlier today. Tried `plink -ssh -batch -pw ... sysadmin@192.168.155.10` (same pattern already proven against `vm-docker`, using the password the user had already shared) — worked immediately once the host key was pinned (`SHA256:TyfqzaNalwIjZCDdDEAv4uPQyZPgL2RiNir9uBdqL4E`).

- Before landing on this, spent a significant chunk of the session chasing WinRM as a Windows-to-Windows alternative to SSH, at the user's suggestion, since they were actively available to approve elevation prompts:
  - Enabled `Enable-PSRemoting -Force -SkipNetworkProfileCheck` on `vm-lab1` via the VNC console — succeeded, confirmed with `Test-WSMan localhost`.
  - On the workstation side, `Set-Item WSMan:\localhost\Client\TrustedHosts` needs elevation this session doesn't have. Triggered UAC elevation directly via `Start-Process powershell -Verb RunAs` with the user approving each prompt live (five rounds total) rather than asking them to type commands themselves.
  - Diagnosed through successive failures, each confirmed via elevated diagnostic scripts logging to `%TEMP%`: (1) `Set-WSManQuickConfig` failing with a WSManFault blaming a "Public profile" — traced to `Ethernet 2` being categorized Public (`Get-NetConnectionProfile`); flipped it to Private — no change; (2) discovered `NlaSvc` (Network Location Awareness) was `Stopped`/`Manual` — started and set to Automatic — no change; (3) confirmed the same fault occurs on a bare read-only `winrm enumerate winrm/config/Listener`, meaning it's not really about the firewall-rule-creation step Enable-PSRemoting normally does, but a client-side gate that runs before any WSMan operation; concluded the real trigger is almost certainly VMware's VMnet1/VMnet8 virtual adapters, which have no default gateway and so never receive *any* network-profile category (not even Public) — outside what `Get-NetConnectionProfile`/`Set-NetConnectionProfile` can even see or fix, since those cmdlets only enumerate categorized interfaces.
  - The documented override for this is a registry policy — `HKLM:\SOFTWARE\Policies\Microsoft\Windows\WinRM\Service\AllowAutoConfig` (+`IPv4Filter`/`IPv6Filter`), the same setting the "Allow automatic configuration of listeners" Group Policy writes. Attempting to write it was **blocked by the harness's own permission classifier** as a security-policy change, and no attempt was made to route around that — surfaced to the user as a real choice (apply the policy themselves, or abandon WinRM) rather than finding another workaround.
  - User chose to drop WinRM. Cleaned up all `%TEMP%\ns7-winrm-*` diagnostic scripts/logs from the workstation. **Left `Ethernet 2` set to Private** (changed mid-diagnosis, no longer needed for the abandoned WinRM approach) — flagged explicitly to the user in case that adapter was intentionally Public.
- **Retrofitted `scripts/devbox.config.ps1`** to match `devserver.config.ps1`'s pattern: added a pinned `HostKey` fingerprint, `Read-DevBoxPassword` (env-var fallback: `NS7_DEVBOX_PASSWORD`), and `Invoke-DevBox`/`Copy-ToDevBox`/`Copy-FromDevBox` helpers over `plink`/`pscp`. Kept the `SshKey` field, documented as currently unusable, in case the upstream signing bug is ever fixed.
- **Rewrote `dev-client.ps1`, `devbox-ui.ps1`, and `provision-devbox.ps1`** to use these helpers instead of plain `ssh`/`scp` (`devbox-ui.ps1` also had its own local `Invoke-DevBox` function shadowing the config's — removed). Removed the now-dead `-SshKey` script parameters. Validated every rewritten script with `ParseFile` before testing live.
- **Validated the whole chain live against `vm-lab1`**: `provision-devbox.ps1 verify` connected via `plink`, ran the base64-encoded inventory probe, and correctly reported everything missing on the freshly bootstrapped box (`git`/`rustc`/`cargo`/`protoc`/`msvc` all `MISSING` — expected, matches the true state).
- **Ran `provision-devbox.ps1 install` for real** and let it complete: Git 2.55.0, VS Build Tools 2022 (VCTools + Windows 11 SDK, `14.44.35207` toolset), Rust pinned to `stable-x86_64-pc-windows-msvc` (rustc/cargo 1.97.1), protoc 35.1 — matching lab1's original inventory exactly. `~15-20` minutes, mostly the Build Tools install.
- **Closed out the full migration end to end**:
  - `dev-client.ps1 -NoRun`: synced source over `pscp`, built `client.exe` on `vm-lab1` from a cold cargo cache (`tao`, `webview2-com`, etc.) in 1m30s — confirms the MSVC/webview toolchain transferred correctly.
  - `devbox-ui.ps1 install`: deployed the harness and registered the `NS7-UITest` scheduled task via `plink`/`pscp`, no `ssh`/`scp` involved at any point.
  - `devbox-ui.ps1 status`: triggered the task against the real interactive session left over from the day's VNC work, captured `window.png`/`screen.png`/`tray.png`/`controls.txt`/`result.json`, copied all five back over `pscp`. Reviewed `window.png` — the client UI renders correctly (dark sidebar, teal accent, "Not connected" / unenrolled state, as expected for a box that hasn't run through setup yet).
  - This is the first time the full pipeline (sync → build → deploy harness → trigger → capture → retrieve) has run against `vm-lab1` using exclusively `plink`/`pscp`, with zero dependency on SSH key auth or WinRM.

### DEV client box migrated: `lab1` -> `vm-lab1`

User is decommissioning the physical box `lab1` (tailnet `100.105.95.89`) and replacing it with a clean Windows 11 Pro VM, `vm-lab1` (`192.168.155.128`).

- **Inventoried the outgoing box rather than working from memory** — it was still up, so the target state was read off it directly over SSH:
  ```powershell
  ssh -i ~\.ssh\id_ed25519_lab1 sysadmin@100.105.95.89 <probe>
  ```
  Result: git 2.55.0, rustc/cargo 1.97.1, protoc 35.1, `stable-x86_64-pc-windows-msvc`, Visual Studio Build Tools 2022, scheduled task `NS7-UITest` (Interactive/Limited, running `C:/dev/ns7-uitest/harness.ps1`), working dirs `C:\dev\nano-stack-7` and `C:\dev\ns7-uitest`, client state in `%LOCALAPPDATA%\NanoStack7`. Notably **node/npm and WiX were absent** — the Admin Console builds inside the server container and the MSI is built by CI, so neither belongs on this box. Provisioning the new one therefore transfers what was actually there rather than inventing requirements.
- **Established the network topology before assuming it.** `192.168.155.128` turned out to be a guest on this workstation's VMware **VMnet8** NAT network (this PC is `192.168.155.1` there), not a tailnet peer. Two consequences the old scripts got wrong:
  - `dev-client.ps1` defaulted the client's server address to this PC's Tailscale IP. The VM has no tailnet membership, so it would have failed to enroll with no obvious cause.
  - `setup-dev-networking.ps1` scoped its firewall rules to the `Private`/`Domain` profiles. `Get-NetConnectionProfile` showed VMnet8 carries **no profile at all** (only Tailscale=Private and Ethernet 2=Public are listed), so those rules would never have matched the VM's traffic.
- **Generated a dedicated keypair** `~\.ssh\id_ed25519_vmlab1` (ed25519, comment `ns7-devbox-vmlab1`); lab1's key retires with lab1.
- **Added `scripts/devbox.config.ps1`** as the single source of truth for host/user/key/remote paths, dot-sourced by every dev script, with per-value `NS7_DEVBOX_*` environment overrides. This migration was a find-and-replace across four scripts plus the README; the next one is a one-line edit. Also added `Resolve-DevBoxServerHost`, which picks whichever of this PC's addresses shares a `/24` with the dev box and falls back to the tailnet IP for a genuinely remote box — correct for a VMnet guest, a LAN peer and a tailnet peer alike. Verified it resolves `192.168.155.1`, and that the env override path works.
- **Added `scripts/bootstrap-devbox.ps1`** — the one unavoidable manual step, run on the new box's own console elevated, because a clean Windows install has no SSH server to let us in. Installs the OpenSSH Server capability, sets the SSH default shell to PowerShell (the default is `cmd.exe`, which mangles the multi-line commands these scripts send), appends the workstation's public key to `administrators_authorized_keys` written as **ASCII with no BOM** (sshd silently ignores a BOM'd file), locks that file down with `icacls /inheritance:r` to Administrators+SYSTEM (sshd refuses to read it otherwise), and opens TCP 22. Also flips any `Public` connection profile to `Private` first — a fresh install classifies an unknown network as Public, which would have left sshd running but unreachable behind the profile-scoped rule.
- **Added `scripts/provision-devbox.ps1`** — idempotent remote toolchain install over SSH (`install` / `verify` actions): Git, VS Build Tools 2022 with `Microsoft.VisualStudio.Workload.VCTools` + `Windows11SDK.22621`, rustup pinned to `stable-x86_64-pc-windows-msvc`, protoc; optional `-IncludeTailscale`. All `winget` calls carry `--accept-source-agreements --disable-interactivity`, without which winget blocks on an unanswerable prompt over SSH. Nothing trusts an exit code — winget and cargo write progress to stderr, which PowerShell-over-SSH surfaces as error records on success — so each step re-probes for the actual artifact, and the inventory probe is sent as a single `-EncodedCommand` round-trip rather than one SSH handshake per tool.
- **Retargeted the existing scripts.** `git mv scripts/lab1-ui.ps1 scripts/devbox-ui.ps1` and `git mv scripts/lab1-ui-harness.ps1 scripts/devbox-ui-harness.ps1` (rename preserved in history); renamed `Invoke-Lab1` -> `Invoke-DevBox` and the `$Lab1Host`/`$Lab1User` parameters throughout `devbox-ui.ps1` and `dev-client.ps1`, both now dot-sourcing the shared config. `dev-client.ps1` also gained a `New-Item -Force` on the remote dir, since `scp` into a non-existent directory on a clean box fails.
- **Rescoped the firewall rules** in `setup-dev-networking.ps1` from `-Profile Private,Domain` to `-Profile Any -RemoteAddress <devbox /24>` — more reliable against a profile-less adapter *and* tighter, since the ports stay closed to every other network this PC is attached to. Existing rules are re-pointed with `Set-NetFirewallRule` rather than reported as "already exists" and left broken. `dev-stack.ps1 status` now advertises the resolved address; verified it prints `192.168.155.1` instead of the tailnet IP.
- **Validated every touched script** with `[System.Management.Automation.Language.Parser]::ParseFile` and an ASCII-only byte check before considering it done — the remote box runs Windows PowerShell 5.1, where a single non-ASCII character is a fatal parse error whose only symptom is a scheduled task exiting 1 with no artifacts.
- Updated `README.md`: Section 13.5 gains a "dev client box" subsection covering the VMnet8 topology and the two-step replacement procedure, the scripts table gains the two new scripts, and Section 13.6's harness commands are renamed. The `lab1` references in Section 11.1's milestone records were **left alone** — those are historical statements of where something was verified, and rewriting them would falsify the record.
