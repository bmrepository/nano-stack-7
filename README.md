...
- **PoC installer distribution (2026-08-02)**: `client/wix/main.wxs` (WiX Toolset v3, via `cargo-wix`) packages `client.exe` + both helper binaries + the tray icon assets into one MSI. Built and published by `.github/workflows/release-client.yml` on a `v*` tag push, using GitHub's `windows-latest` runner (MSVC + WiX v3 both preinstalled — no dependency on any particular dev machine to cut a release) and publishing the MSI as a GitHub Release asset. The Admin Console's Workspaces page links to `.../releases/latest/download/nano-stack-7-client-installer.msi`, a stable URL that always resolves to the newest release. **Unverified end-to-end** — authored without a working local WiX build to test against this session; expect the first real tag push to need at least one iteration based on the Actions log.
...
- **Confirmed working (2026-08-02)**: running the client directly on a machine's own interactive desktop session shows the tray icon correctly. Running it via SSH does **not** — Windows session isolation means an SSH-launched process's UI lands in a different, non-visible session than the one you're physically looking at. This affects `consent-helper` too, not just the tray icon.
...
- **Workspace/server config is in-memory** (revised 2026-08-02): `server/src/workspace.rs`'s `ServerIdentity` loads from `SERVER_PRIVATE_KEY_HEX` (ephemeral random fallback, logged warning if unset); `WorkspaceStore` holds workspaces created via the Admin Console (`id`/`name`/`created_at`, no crypto key). This stands in for the real Postgres-backed Workspace Manager (Section 4.1) — nothing here survives a server restart until that lands.
